function cam_properties(_num) {
	var _cam = view_camera[_num];
	...
	return {
		...
		xw: camera_get_view_x(_cam) + camera_get_view_width(_cam),
		yh: camera_get_view_y(_cam) + camera_get_view_height(_cam),
		angle: camera_get_view_angle(_cam),
	};
}

function get_key_str(_key) {
	switch(_key) {
