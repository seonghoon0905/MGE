...
function handle_animation_value(){
	...
	var _value = obj_world.settings_item_ui_anim_time;
	if(_value < 1){
		obj_world.settings_item_ui_anim_time = min(1, _value + _transition_spd);
	}
}

function find_item_icon_index(_index){
	switch(global.other_player_data.item[_index]){
		case "pop_gun":
			return 0;
		case "machine_gun":
			return 1;
...
function make_graph_surf(){ 
	...
	var _pos_x = 0;
	for(var _index = 0; _index <= 2 * array_length(global.other_player_data.item); _index++){
		if(_index % 2 == 1){
			draw_sprite(spr_item_icon, find_item_icon_index((_index - 1) / 2), _pos_x, 0);
			_pos_x += icon_width;
		}
		else{
...
function make_final_surf(){
	...
	to_x = (icon_width + gap_between_icons) * 3 - obj_world.settings_item_index * (icon_width + gap_between_icons);
	draw_x = lerp(draw_x, to_x, obj_world.settings_item_ui_anim_time);
