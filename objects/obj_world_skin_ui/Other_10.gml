...
function handle_animation_value(){
	...
	var _value = obj_world.settings_skin_ui_anim_time;
	if(_value < 1){
		obj_world.settings_skin_ui_anim_time = min(1, _value + _transition_spd);
	}
}

function get_skin_idle(_index){
	switch(global.other_player_data.skins[_index]){
		case "standard":
			return spr_standard_idle;
		case "crimson":
			return spr_crimson_idle;
		case "round":
			return spr_round_idle;
	}
}
...
function make_graph_surf(){ 
	...
	graph_surf = surface_create(array_length(global.other_player_data.skins) * (icon_width + gap_between_icons), icon_height);
	surface_set_target(graph_surf);
	var _pos_x = 0;
	for(var _index = 0; _index <= 2 * array_length(global.other_player_data.skins); _index++){
		if(_index % 2 == 1){
			draw_sprite(get_skin_idle((_index - 1) / 2), 0, _pos_x + 17, 21);
			_pos_x += icon_width;
		}
		else{
...
function make_final_surf(){
	...
	to_x = (icon_width + gap_between_icons) * 3 - obj_world.settings_skin_index * (icon_width + gap_between_icons);
	draw_x = lerp(draw_x, to_x, obj_world.settings_skin_ui_anim_time);
