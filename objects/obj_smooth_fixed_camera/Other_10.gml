...
function set_fixed_camera_position(){
	...
	if(keyboard_check_pressed(global.key_config.load)){
		...
		var _px = (_x div DEFAULT_CAMERA_WIDTH) * DEFAULT_CAMERA_WIDTH;
		var _py = (_y div DEFAULT_CAMERA_HEIGHT) * DEFAULT_CAMERA_HEIGHT;
		x = _px;
		y = _py;
	}
	else{
		...
		var _px = (_x div DEFAULT_CAMERA_WIDTH) * DEFAULT_CAMERA_WIDTH;
		var _py = (_y div DEFAULT_CAMERA_HEIGHT) * DEFAULT_CAMERA_HEIGHT;
		x += (_px - x) / follow_latency;
		y += (_py - y) / follow_latency;
	}
}
